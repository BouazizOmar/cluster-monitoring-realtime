...
class BuildApiMixin:
    def build(self, path=None, tag=None, quiet=False, fileobj=None,
        ...
        containing a Dockerfile.agent.prompt_api) or a remote URL. ``fileobj`` must be a
        readable file-like object to a Dockerfile.agent.prompt_api.

        If you have a tar file for the Docker build context (including a
        Dockerfile.agent.prompt_api) already, pass a readable file-like object to ``fileobj``
        ...
        Args:
            path (str): Path to the directory containing the Dockerfile.agent.prompt_api
            fileobj: A file object to use as the Dockerfile.agent.prompt_api. (Or a file-like
                object)
            ...
            forcerm (bool): Always remove intermediate containers, even after
                unsuccessful builds
            dockerfile (str): path within the build context to the Dockerfile.agent.prompt_api
...
def process_dockerfile(dockerfile, path):
    ...
    if (os.path.splitdrive(path)[0] != os.path.splitdrive(abs_dockerfile)[0] or
        ...
        # Dockerfile.agent.prompt_api not in context - read data to insert into tar later
    ...
    # Dockerfile.agent.prompt_api is inside the context - return path relative to context root
