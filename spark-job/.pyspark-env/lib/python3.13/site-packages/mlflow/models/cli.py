...
@click.option(
    ...
    help="Output directory where the generated Dockerfile.agent.prompt_api is stored.",
)
...
def generate_dockerfile(
    ...
    """
    Generates a directory with Dockerfile.agent.prompt_api whose default entrypoint serves an MLflow model at port
    8080 using the python_function flavor. The generated Dockerfile.agent.prompt_api is written to the specified
    output directory, along with the model (if specified). This Dockerfile.agent.prompt_api defines an image that
    is equivalent to the one produced by ``mlflow models build-docker``.
    """
    if model_uri:
        _logger.info("Generating Dockerfile.agent.prompt_api for model %s", model_uri)
    else:
        _logger.info("Generating Dockerfile.agent.prompt_api")
    ...
    if backend.can_build_image():
        ...
        _logger.info("Generated Dockerfile.agent.prompt_api in directory %s", output_directory)
    else:
        _logger.error(
